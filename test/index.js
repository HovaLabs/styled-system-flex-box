...
import { Absolute, AbsoluteFlex, Box, Flex, Text } from "../index";
...
describe("Absolute", () => {
  test("renders", () => {
    const json = renderJSON(<Absolute />);
    expect(json.type).toBe("div");
  ...
  test("renders with absolute props", () => {
    const json = renderJSON(<Absolute top="auto" />);
    expect(json).toHaveStyleRule("top", "auto");
  });

  test("renders with Box props", () => {
    const json = renderJSON(<Absolute color="tomato" />);
    expect(json).toHaveStyleRule("color", "tomato");
  });

  test("renders with as and Box props", () => {
    const json = renderJSON(<Absolute as="footer" color="tomato" />);
    expect(json.type).toBe("footer");
    expect(json).toHaveStyleRule("color", "tomato");
  });
});
...
describe("AbsoluteFlex", () => {
  test("renders", () => {
    const json = renderJSON(<AbsoluteFlex />);
    expect(json.type).toBe("div");
    expect(json).toHaveStyleRule("position", "absolute");
    expect(json).toHaveStyleRule("top", "0");
    expect(json).toHaveStyleRule("bottom", "0");
  ...
  test("renders with absolute props", () => {
    const json = renderJSON(<AbsoluteFlex top="auto" />);
    expect(json).toHaveStyleRule("top", "auto");
  });

  test("renders with Flex props", () => {
    const json = renderJSON(<AbsoluteFlex alignItems="center" />);
    expect(json).toHaveStyleRule("align-items", "center");
  });
...
describe("Text", () => {
  test("renders", () => {
    const json = renderJSON(<Text />);
    expect(json.type).toBe("p");
  });

  test("renders with text props", () => {
    const json = renderJSON(<Text lineHeight="16px" />);
    expect(json).toHaveStyleRule("line-height", "16px");
  });

  test("renders with Box props", () => {
    const json = renderJSON(<Text color="tomato" />);
    expect(json).toHaveStyleRule("color", "tomato");
  });

  test("renders with as and Box props", () => {
    const json = renderJSON(<Text as="footer" color="tomato" />);
    expect(json.type).toBe("footer");
    expect(json).toHaveStyleRule("color", "tomato");
  });
});
